...
lint:
	CMD="lint charts/cray-s3" $(MAKE) helm

dep-up:
	CMD="dep up charts/cray-s3" $(MAKE) helm
...
ifdef CHART_VERSIONS
	CMD="package charts/cray-s3 --version $(word 1, $(CHART_VERSIONS)) -d packages" $(MAKE) helm
else
...
images:
	CHART=charts/cray-s3 $(MAKE) -s extracted-images annotated-images | sort -u
...
clean:
	$(RM) -r .helm packages charts/cray-s3/charts
